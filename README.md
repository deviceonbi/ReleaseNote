...
- While an alarm (event) occurs, allows the user to monitor the sequence of alarm (event) and parameters related to the alarm (event).
- User can configure the Number of sample before/after event and the sample rate (1~3600 seconds).
- User can configure the related parameters.
- After the configuration, user need to use "Ene Alarm Panel" of WISE-PaaS Dashboard and the "alarmlog_record" function of deviceon-bi-simplejson datasource to display the result of SOE.

#### Negative mode of Cumulative parameter

- While compute the difference of data (HIS_AVG) of cumulative parameter:
...
- After alarm rule change, check if the current value match the rule or not immediately.
...
- [Bug] Call Inspection API with self signed certificate issue.

#### Inspection

- [Bug] On-Premise version of Inspection cannot connect to local Blob.

#### Evaluator

- 優化計算線程, 讓計算能確實依calculation rate計算

#### Packer

- [Bug] While query a data that is not in Blob, the API call will timeout.
  
#### Datasource backend

- Adjust the upper limit of Request body size.

#### Archiver

- Redundant calculations may cause errors in day/month/year data while the Archiver restart.

#### Portal

- [Bug] Cannot save parameter cause that the portal got wrong Multi-lang. structure.
